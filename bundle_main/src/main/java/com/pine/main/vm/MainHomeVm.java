...
public class MainHomeVm extends ViewModel {
    ...
    public void loadBusinessBundleData() {
        ...
        try {
            ...
            if (ConfigSwitcherServer.getInstance().isEnable(ConfigKey.BUNDLE_BUSINESS_DEMO_KEY)) {
                jsonObject = new JSONObject();
                jsonObject.put("name", "Business Demo");
                jsonObject.put("bundle", ConfigKey.BUNDLE_BUSINESS_DEMO_KEY);
                jsonObject.put("command", RouterDemoCommand.goDemoHomeActivity);
                jsonArray.put(jsonObject);
            }
